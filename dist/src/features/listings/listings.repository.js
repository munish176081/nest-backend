    ...
    async findByUserId(userId, options) {
        ...
        if (listingIds.length > 0) {
            const subscriptions = await this.subscriptionRepository.find({
                where: { id: (0, typeorm_2.In)(listingIds), status: subscription_entity_1.SubscriptionStatusEnum.ACTIVE },
                select: ['id', 'currentPeriodEnd'],
            });
            const subscriptionMap = new Map(subscriptions.map(s => [s.id, s.currentPeriodEnd]));
