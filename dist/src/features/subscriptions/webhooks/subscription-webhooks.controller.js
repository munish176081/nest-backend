    ...
    async handlePayPalSubscriptionActivated(event) {
        ...
        if (subscription.listingId) {
            try {
                ...
                if (listing) {
                    if (listing.status === listing_entity_1.ListingStatusEnum.DRAFT || !listing.isActive) {
                        const updateData = {
        ...
        if (subscription.listingId) {
            ...
            if (inactiveStatuses.includes(status)) {
                ...
                try {
                    ...
                    if (listing && (listing.status === listing_entity_1.ListingStatusEnum.DRAFT || !listing.isActive)) {
                        const updateData = {
                            status: listing_entity_1.ListingStatusEnum.ACTIVE,
                            isActive: true,
                        };
