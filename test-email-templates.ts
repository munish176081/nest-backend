/**
 * Test script for Postmark email templates
 * 
 * Usage:
 *   ts-node test-email-templates.ts
 * 
 * Or test a specific template:
 *   ts-node test-email-templates.ts email-verification
 * 
 * Make sure your .env file has:
 *   EMAIL_SERVICE_TYPE=postmark
 *   POSTMARK_API_KEY=your-api-key
 *   POSTMARK_EMAIL=your-verified-sender@example.com
 */
...
//   - POSTMARK_API_KEY
//   - POSTMARK_EMAIL
//   - TEST_EMAIL_RECIPIENT (optional, defaults to POSTMARK_EMAIL)

const POSTMARK_API_KEY = process.env.POSTMARK_API_KEY;
// const POSTMARK_EMAIL = process.env.POSTMARK_EMAIL;
...
// Test data for each template - using actual Postmark template aliases
const testData = {
  ...
  // Keep old aliases for backward compatibility (will map to new ones)
  'email-verification': {
    username: 'John Doe',
    verificationUrl: 'https://example.com/auth/verify-email?token=abc123&userId=user-123',
  },
...
async function testTemplate(templateAlias: string) {
  console.log(`\n📧 Testing template: ${templateAlias}`);
  console.log('─'.repeat(50));

  const templateData = testData[templateAlias as keyof typeof testData];
...
async function runAllTests() {
  ...
  // Updated to match actual Postmark template aliases
  const templates = [
    'welcome',          // email-verification
    'password-reset',   // reset-password
    'welcome-1',        // email-verification-otp
